...
Таблица `stock_catalog` создана в production миграцией `20260923154823`; все 23 старые строки `job_parts` импортированы по одной через `legacy_part_id` без объединения повторных SKU. Сейчас каталог содержит 23 активные строки, 5 с артикулом, без сирот; `job_parts` остаются неизменными финансовыми снимками, связанными с заявками. Вкладка «Склад» опубликована и проверена. Следующая миграция добавляет к `service_order_items` явный тип строки, ID позиции каталога и снимки SKU/цены/себестоимости; новая/изменённая номенклатура копирует значения из активного каталога, повторное сохранение сохраняет прежний финансовый снимок. Перенос остатка задания копирует снимок. `work_catalog.materials` пока остаётся JSON, подтверждаемое обновление снимка ещё предстоит реализовать; физические остатки и движения не ведутся. Начальный физический остаток нельзя выводить из старых строк расхода.
...
PR #62 добавил связи «заявка — задание — выезд». PR #63 добавил подтверждаемое распределение стоянок и километров на задания, неизменяемый журнал расчётов, проверку устаревания при изменении факта и свежие сводки затрат в задании и заявке. Миграция применена к production, 18 исторических долей стоянок сохранены без изменения факта; CI и деплой прошли. Себестоимость километража рассчитывается только после подтверждения выезда; суточные, ночлег и ручная корректировка пока видны как нераспределённые. Распределение выручки не менялось. Базовая оргструктура поверх `profiles` добавлена в production; UI должен попасть в ближайший deploy. Остаются назначения владельца/куратора/исполнителей, история оргназначений, каталог/склад и отдельное решение по фиксированным затратам.
