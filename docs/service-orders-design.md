...
Первый технический слой уже добавляет `employee_org` поверх `profiles.id`: должность и один прямой руководитель на сотрудника. Новые профили автоматически получают пустую запись оргструктуры; текущие профили были перенесены без выдумывания должностей и руководителей. Администратор редактирует имя, роль, должность и руководителя в разделе «Сотрудники и роли». Руководитель должен быть активным пользователем, самоназначение и циклы запрещены. Передача владельца при выбытии, кадровая история и назначения владельца/куратора/исполнителей пока не реализованы.
