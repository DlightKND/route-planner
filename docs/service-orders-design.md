...
Таблица `stock_catalog` создана в production миграцией `20260923154823`; все 23 старые строки `job_parts` импортированы по одной через `legacy_part_id` без объединения повторных SKU. Сейчас каталог содержит 23 активные строки, 5 с артикулом, без сирот; `job_parts` остаются неизменными финансовыми снимками, связанными с заявками. UI-вкладка ожидает публикации. Выбор каталога в задании и его снапшоты ещё предстоит подключить. Номенклатура из `work_catalog.materials` пока остаётся JSON, а физические остатки и движения не ведутся. Начальный физический остаток нельзя выводить из старых строк расхода.
