...
Аудит `src/app.js` обнаружил, что заявка сохраняла `job_works` как `DELETE всех → INSERT заново`. Это уничтожало ID и временные поля каждого work row даже при сохранении остальных полей заявки, поэтому такой ID пока нельзя использовать как устойчивый provenance key для задания. PR #72 `fix/stable-job-work-rows` переводит редактор и offline queue на сопоставление строк по ID: неизменённые строки не пишутся повторно, изменённые сохраняют ID, удаляются только строки, исключённые из текущего состава; правка менеджера получает новое подтверждение, а неизменённый `revenue` не пересчитывается при постороннем редактировании заявки. Backfill старых работ и материалов не начинать до слияния и проверки этого PR. Это закрывает только стабильность ID; дальнейший перевод чтений/записей, старых отчётов и план-факта на `service_order_items` ещё обязателен.
