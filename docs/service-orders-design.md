...
PR #62 добавил связи «заявка — задание — выезд» и основу для автоматизации учёта. Следующий этап добавляет подтверждаемое распределение стоянок и километров на задания, неизменяемый журнал расчётов, проверку устаревания при изменении факта и отображение свежих распределений в задании и заявке. Себестоимость километража рассчитывается только после подтверждения выезда; суточные, ночлег и ручная корректировка пока видны как нераспределённые. Этот этап не меняет распределение выручки. Тесты миграции проходят на PGlite; продуктивную схему и интерфейс считать обновлёнными только после применения миграции, успешного CI и публикации сайта.
