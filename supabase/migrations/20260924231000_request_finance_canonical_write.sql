-- New editor generations write request finance directly to the unique seed
-- task. The older job_request_save RPC remains available for queued clients.
begin;

create function dlight_private.request_finance_save(
...
begin
  ...
  if p_works is not null then
    for item,item_no in select value,ordinality from jsonb_array_elements(p_works) with ordinality loop
      ...
      if prior.id is null then
        ...
        values(item_id,oid,jid,title,unit,qty,'work',work_id,billable,reason,profile,
          ...
          jsonb_build_object('request_finance_generation',1,'revenue_override',override_value));
      else
    ...
    if exists(select 1 from public.service_order_items i where i.order_id=oid and i.job_id=jid and i.kind='work'
       and i.legacy_job_work_id is null and i.legacy_job_part_id is null and not(i.id=any(kept))
       and (i.done_qty>0 or i.transferred_qty>0 or i.source_item_id is not null)) then
      raise exception 'Нельзя удалить работу с результатом или переносом';
    end if;
  ...
  if p_parts is not null then
    for item,item_no in select value,ordinality from jsonb_array_elements(p_parts) with ordinality loop
      ...
      if item_id is not null then
        ...
        if not manager and prior.approved_at is not null and
          (prior.title,prior.unit,prior.planned_qty,prior.sku_snapshot,billable)
          is distinct from (part_name,part_unit,part_qty,part_sku,prior.billable) then
    ...
    if exists(select 1 from public.service_order_items i where i.order_id=oid and i.job_id=jid and i.kind='material'
       and i.legacy_job_work_id is null and i.legacy_job_part_id is null and not(i.id=any(kept))
       and (i.done_qty>0 or i.transferred_qty>0 or i.source_item_id is not null)) then
      raise exception 'Нельзя удалить материал с результатом или переносом';
    end if;
...
-- Request-owned canonical finance rows stay in the task read model, but the
-- dispatch-plan editor cannot rewrite or remove them as ordinary task scope.
create function dlight_private.guard_request_finance_task_item()
returns trigger language plpgsql set search_path='' as $$
begin
  if coalesce(current_setting('dlight.request_finance_sync',true),'')='on' then
    return case when tg_op='DELETE' then old else new end;
  end if;
  if old.legacy_snapshot->>'request_finance_generation'='1' then
    if tg_op='DELETE' then raise exception 'Строку заявки меняют только через редактор заявки'; end if;
    if (new.order_id,new.job_id,new.title,new.unit,new.planned_qty,new.kind,new.stock_catalog_id,
        new.sku_snapshot,new.unit_price_snapshot,new.unit_cost_snapshot,new.work_catalog_id,new.billable,
        new.billable_reason,new.tariff_profile,new.financial_revenue_snapshot,new.financial_cost_snapshot,
        new.approved_at,new.approved_by,new.legacy_snapshot)
       is distinct from
       (old.order_id,old.job_id,old.title,old.unit,old.planned_qty,old.kind,old.stock_catalog_id,
        old.sku_snapshot,old.unit_price_snapshot,old.unit_cost_snapshot,old.work_catalog_id,old.billable,
        old.billable_reason,old.tariff_profile,old.financial_revenue_snapshot,old.financial_cost_snapshot,
        old.approved_at,old.approved_by,old.legacy_snapshot) then
      raise exception 'Строку заявки меняют только через редактор заявки';
    end if;
  end if;
  return new;
end $$;
revoke all on function dlight_private.guard_request_finance_task_item() from public,anon,authenticated;
drop trigger if exists service_order_items_request_finance_guard on public.service_order_items;
create trigger service_order_items_request_finance_guard before update or delete on public.service_order_items
for each row execute function dlight_private.guard_request_finance_task_item();

create function public.job_request_save_canonical(p_id uuid,p_rec jsonb,p_works jsonb default null,p_parts jsonb default null)
returns jsonb language sql security invoker set search_path='' as $$
  select dlight_private.request_finance_save(p_id,p_rec,p_works,p_parts)
$$;
revoke all on function dlight_private.request_finance_save(uuid,jsonb,jsonb,jsonb) from public,anon,authenticated;
grant execute on function dlight_private.request_finance_save(uuid,jsonb,jsonb,jsonb) to authenticated;
revoke all on function public.job_request_save_canonical(uuid,jsonb,jsonb,jsonb) from public,anon;
grant execute on function public.job_request_save_canonical(uuid,jsonb,jsonb,jsonb) to authenticated;

create function dlight_private.request_finance_approve(p_job uuid,p_ids uuid[])
...
begin
  ...
  update public.service_order_items set approved_at=now(),approved_by=auth.uid()
   where order_id=oid and job_id=p_job and id=any(coalesce(p_ids,'{}'::uuid[])) and approved_at is null
     and legacy_job_work_id is null and legacy_job_part_id is null;
  get diagnostics changed=row_count;
  if changed<>cardinality(coalesce(p_ids,'{}'::uuid[])) then raise exception 'Не все строки доступны для подтверждения'; end if;
  perform set_config('dlight.request_finance_sync','off',true);
  return changed;
end $$;
create function public.job_request_finance_approve(p_job uuid,p_ids uuid[])
returns integer language sql security invoker set search_path='' as $$
  select dlight_private.request_finance_approve(p_job,p_ids)
$$;
revoke all on function dlight_private.request_finance_approve(uuid,uuid[]) from public,anon,authenticated;
grant execute on function dlight_private.request_finance_approve(uuid,uuid[]) to authenticated;
revoke all on function public.job_request_finance_approve(uuid,uuid[]) from public,anon;
grant execute on function public.job_request_finance_approve(uuid,uuid[]) to authenticated;
