-- A01/A02: preserve safe settings reads; deny writes and inactive RPC callers.
-- Based on the inspected production definitions from 2026-09-21.
begin;
revoke all on public.settings_public from public, anon, authenticated;
...
begin
  for c in select attname from pg_attribute
    where attrelid='public.settings_public'::regclass and attnum>0 and not attisdropped
  loop
    execute format('revoke insert (%I), update (%I), references (%I) on public.settings_public from public, anon, authenticated',c.attname,c.attname,c.attname);
  end loop;
end $guard$;
...
AS $function$
begin
  if coalesce(public.user_role(),'') not in ('admin','logist') then
...
begin
  ...
  if p_kind = 'jobs' then
    ...
    perform 1 from public.trips where id=p_id and deleted_at is not null for update;
    if not found then raise exception 'Выезд не находится в корзине'; end if;
...
begin
  begin
    select * into t from trips where id=p_trip and deleted_at is null for update nowait;
  exception when lock_not_available then return 'busy'; end;
  if not found then return 'not_found'; end if;
  if not coalesce(public.user_role() is not null and (public.is_owner_or_mgr(t.lead_engineer) or auth.uid()=any(t.engineer_ids)),false) then raise exception 'Это не ваш выезд'; end if;
...
begin
  if coalesce(public.user_role(),'') not in ('admin','logist') then raise exception 'Недостаточно прав'; end if;
...
revoke execute on function public.is_owner_or_mgr(uuid), public.soft_delete_client(uuid),
 public.restore_deleted(timestamptz), public.trash_restore(text,uuid), public.trash_delete_forever(text,uuid),
 public.trip_tracking_cancel(uuid), public.trip_tracking_reassign(uuid,uuid),
 public.vehicle_odometer_set(uuid,numeric,text), public.trip_start(uuid), public.trip_finish(uuid)
 from public, anon;
grant execute on function public.is_owner_or_mgr(uuid), public.soft_delete_client(uuid),
 public.restore_deleted(timestamptz), public.trash_restore(text,uuid), public.trash_delete_forever(text,uuid),
 public.trip_tracking_cancel(uuid), public.trip_tracking_reassign(uuid,uuid),
 public.vehicle_odometer_set(uuid,numeric,text), public.trip_start(uuid), public.trip_finish(uuid)
 to authenticated;
