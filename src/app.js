...
const { money, hhmm, businessDays, jobRoadPayer, rateFrom, dedupeStops, tspOrder,
        ...
        planSchedule, scheduleJobIncluded, tripRouteSegments, driveOfLegs, piecesOf, normPos, addHours, diffHours, clockOf, dayScaleBounds, dayWindow, weekRowSpan, q4,
        trashDaysLeft,
        measureTrip } = core;
...
function gtDayBounds(key,iso){
  const pieces=[]; gtBlocks().forEach(b=>pieces.push(...gtPieces(b).filter(p=>p.iso===iso)));
  return dayScaleBounds(pieces,+appSettings.day_start||7,gtDayScale[gtScaleKey(key,iso)]);
}
function gtHourOptions(selected){
  let out=''; for(let h=0;h<=24;h+=.5){ const m=h%1?'30':'00', label=String(Math.floor(h)%24).padStart(2,'0')+':'+m;
    out+='<option value="'+h+'"'+(Math.abs(h-selected)<.01?' selected':'')+'>'+label+'</option>'; }
  return out;
}
...
function gtNowLine(cols,eff){
  ...
  const left=on?((i*eff+rel)/(cols.length*eff)*100):0;
...
function paintScheduleNow(){
  ...
  document.querySelectorAll('[data-gnow]').forEach(el=>{
    ...
    const rel=n.t-(+appSettings.day_start||7),on=i>=0&&rel>=0&&rel<=eff;
    el.style.display=on?'':'none';
    if(on){ el.style.left=((i*eff+rel)/(cols.length*eff)*100)+'%'; el.title='Сейчас · '+n.label; }
  });
  document.querySelectorAll('[data-wknow]').forEach(el=>{
    const rel=n.t-(+appSettings.day_start||7),on=el.dataset.wknow===n.iso&&rel>=0&&rel<=shift;
    el.style.display=on?'':'none';
...
function gtDayHtml(key){
  ...
  const eff=gtEff(), shift=((+appSettings.day_end)||16)-((+appSettings.day_start)||7), st=gtSettings(), bounds={from:0,to:24};
  const span=24, relFrom=0, dayH=330;
  const lanes=gtWeekLanes(key), laneMin=window.matchMedia('(max-width:600px)').matches?120:150;
  const rowN=Math.max(1,Math.ceil(span));
  let axis='<span class="vg-axis-head"></span>';
  for(let h=0;h<rowN;h++) axis+='<span class="vg-hour-label">'+clockLabel(h)+'</span>';
  ...
  lanes.forEach(l=>{
    ...
    const w=dayWindow(l.id,iso,st); let ticks=''; for(let h=1;h<rowN;h++) ticks+='<i class="vg-hour-line" style="top:'+(h/span*dayH)+'px"></i>';
    if(w) ticks+='<span class="vg-off" style="top:0;height:'+(w.start/24*dayH)+'px"></span><span class="vg-off" style="top:'+(w.ceiling/24*dayH)+'px;bottom:0"></span>'
      +'<span class="vg-tolerance" style="top:'+(w.end/24*dayH)+'px;height:'+((w.ceiling-w.end)/24*dayH)+'px"></span>'
      +'<button class="vg-win-line start" data-winline="start" data-engineer="'+esc(l.id)+'" style="top:'+(w.start/24*dayH)+'px"><i>начало '+clockLabel(w.start)+'</i></button>'
      +'<button class="vg-win-line end" data-winline="end" data-engineer="'+esc(l.id)+'" style="top:'+(w.end/24*dayH)+'px"><i>конец '+clockLabel(w.end)+'</i></button>'
      +'<button class="vg-win-line tolerance" data-winline="tol" data-engineer="'+esc(l.id)+'" style="top:'+(w.ceiling/24*dayH)+'px"><i>допуск '+clockLabel(w.ceiling)+'</i></button>'
      +(w.proposedEnd!=null?'<span class="vg-proposed" style="top:'+(w.proposedEnd/24*dayH)+'px">предложено '+clockLabel(w.proposedEnd)+'</span>':'');
    let bars='';
    gtBlocks().filter(b=>(b.engineer||' free')===l.id).forEach(b=>{
      ...
      pcs.forEach(p=>{const y=(p.from-relFrom)/span*dayH,h=Math.max(12,p.h/span*dayH),name=p.k==='d'?'Дорога':((p.jobId&&feedCtx.jobName(p.jobId))||gtBlockName(b));
    ...
    const now=scheduleNow(), nowY=(now.t-relFrom)/span*dayH, nowLine=now.iso===iso&&nowY>=0&&nowY<=dayH?'<span class="vg-now" title="Сейчас · '+esc(now.label)+'" style="top:'+nowY+'px"></span>':'';
    tracks+='<div class="vg-lane vg-day-lane" data-vglane="'+esc(l.id)+'" data-vgspan="'+span+'" style="--load:'+color+';--load-edge:'+loadEdge(color)+'">'+ticks+bars+nowLine
...
function gtVerticalHtml(key){
  ...
  days.forEach((iso,i)=>axis+='<button style="height:'+rows[i].px+'px" class="vg-date'+(rows[i].weekend?' we':'')+(iso===today?' today':'')+'" data-gday="'+iso+'"><b>'+WD_RU[(i+1)%7]+'</b> '+shortDate(iso)+'</button>');
  axis='<div class="vg-axis" style="grid-template-rows:34px '+template+'">'+axis+'</div>';
  let heads='',tracks='';
  lanes.forEach(l=>{
    const weekH=days.reduce((n,d)=>n+gtLaneLoad(l.id,d),0),windowH=Math.max(.25,(+appSettings.day_end||16)-(+appSettings.day_start||7));
    heads+='<div class="vg-lane-head"><b>'+esc(l.name)+'</b><span>'+fmtH(weekH)+(feedCtx.mine?'':' · '+Math.round(weekH/(windowH*5)*100)+'%')+'</span></div>';
    let cells='',bars='',offset=0;
    days.forEach((iso,i)=>{const row=rows[i],w=dayWindow(l.id,iso,gtSettings()),hours=gtLaneLoad(l.id,iso),p=hours/(w?Math.max(.25,w.end-w.start):windowH),over=Math.max(0,hours-(w?Math.max(0,w.end-w.start):windowH));
      cells+='<div class="vg-cell'+(row.weekend?' we':'')+(iso===today?' today':'')+'" style="position:absolute;top:'+offset+'px;height:'+row.px+'px">'
  ...
  return '<div class="vg-tools"><span>'+(feedCtx.mine?'Работы по дням':'Рабочее окно '+fmtH((+appSettings.day_end||16)-(+appSettings.day_start||7))+' · допуск '+fmtH(appSettings.tolerance_h||1))+'</span>'+gtLanePicker(key)+'</div>'
...
function gtWire(key,box){
  ...
  const scaleFrom=box.querySelector('[data-vgfrom]'), scaleTo=box.querySelector('[data-vgto]');
  const saveScale=()=>{ const from=+scaleFrom.value,to=+scaleTo.value; if(to<=from){ showToast('Конец масштаба должен быть позже начала'); return; }
    gtDayScale[gtScaleKey(key,zoom)]={from,to}; gtPaint(key); };
  if(scaleFrom) scaleFrom.onchange=saveScale;
  if(scaleTo) scaleTo.onchange=saveScale;
  const auto=box.querySelector('[data-vgauto]'); if(auto) auto.onclick=()=>{ delete gtDayScale[gtScaleKey(key,zoom)]; gtPaint(key); };
  box.querySelectorAll('[data-winline]').forEach(line=>line.onpointerdown=e=>{
    ...
    line.setPointerCapture(e.pointerId);line.onpointermove=ev=>{value=q4(Math.max(0,Math.min(24,(ev.clientY-R.top)/R.height*24)));line.style.top=(value/24*R.height)+'px';line.querySelector('i').textContent=(kind==='tol'?'допуск ':(kind==='start'?'начало ':'конец '))+clockLabel(value);};
  ...
  box.querySelectorAll('.gpc,.vg-block:not(.vg-piece)').forEach(el=>{
    el.onpointerdown=e=>{
      ...
      gtDrag={id:b.id,block:b,start:b.start,base:b.start,x0:e.clientX,
        y0:e.clientY,w:tr.getBoundingClientRect().width,h:tr.getBoundingClientRect().height,
        visibleSpan:+tr.dataset.vgspan||eff,vertical:vertical,
    ...
    el.onpointermove=e=>{
      ...
      const visibleSpan=gtDrag.zoom?gtDrag.visibleSpan:eff;
      const raw=gtDrag.vertical?(e.clientY-gtDrag.y0)/gtDrag.h*visibleSpan:(e.clientX-gtDrag.x0)/gtDrag.w*span;
...
async function gtSave(b,start){
  ...
  if(plan&&b.plan&&Array.isArray(b.plan.cuts)){
    ...
    plan.cuts=b.plan.cuts.map(x=>{ const p=addHours({iso:x.at.d,t:+x.at.t},dh,gtSettings()); return {...x,at:{d:p.iso,t:q4(p.t)}}; });
  }
...
function engineersCount(plan){
  const n=(profilesList||[]).filter(p=>p&&p.role==='engineer'&&p.active!==false&&String(p.full_name||'').trim()).length;
  if(n) return n;
  const lanes={};
...
async function renderFeed(box,o){
  ...
  try{
    ...
    // Гант читает уже посчитанное: своего расчёта у него нет и быть не должно.
    gtApply();
...
//
// Диспетчер ЗАВОДИТ заявку, инженер её ВЫПОЛНЯЕТ. Первому нужна форма,
// второму — задание: к кому, что сломано, записать часы, закрыть.
// Одна страница на обе роли отдавала инженеру полэкрана выпадающих
// списков до первого, что ему нужно, — и позволяла случайно сменить
// у заявки клиента или исполнителя.
// Дата словами. Системный ввод даты показывает её в формате браузера,
// и «09/11/2026» читается и как 9 ноября, и как 11 сентября.
const MON_GEN=['января','февраля','марта','апреля','мая','июня','июля',
  'августа','сентября','октября','ноября','декабря'];
function dateHuman(iso){ if(!iso) return '—';
  const d=new Date(iso+'T00:00:00'); if(isNaN(d)) return iso;
  return d.getDate()+' '+MON_GEN[d.getMonth()]+' '+d.getFullYear(); }
function selText(id){ const s=$(id); if(!s||s.selectedIndex<0) return '—';
  const t=(s.options[s.selectedIndex]||{}).text||''; return t.trim()||'—'; }
function setJobMode(task){
  const v=document.querySelector('.view-job'); if(!v) return;
  v.classList.toggle('job-task',!!task);
  const read=$('jobRefRead'), tg=$('jobRefToggle');
  if(task&&read){
    read.hidden=true;
...
// Пересобирается при каждом обновлении шапки: после «Взять в работу»
// статус в справке должен меняться вместе с кнопкой, а не после
// перезахода на страницу.
function buildJobRead(){
  const read=$('jobRefRead'); if(!read) return;
  const rows=[
      ...
      ['Заметка',$('jbNotes').value.trim()||'—']];
  if(jobAtDepot) rows.splice(2,0,['В депо',selText('jbDepotSel')]);
  read.innerHTML='<dl>'+rows.map(r=>'<dt>'+esc(r[0])+'</dt><dd>'+esc(r[1])+'</dd>').join('')+'</dl>';
}
if($('jobRefToggle')) $('jobRefToggle').onclick=()=>{
  const read=$('jobRefRead'); if(!read) return;
  const open=read.hidden;
  read.hidden=!open;
  $('jobRefToggle').setAttribute('aria-expanded',open?'true':'false');
...
function jobFootUpdate(){
  ...
  $('jobFootHint').textContent=(next==='done'&&!hrs)
    ? 'Часы не записаны — заявка закроется с нулём'
    : (next==='done'?('Записано '+hrs.toFixed(1)+' ч'):'');
}
if($('jobPrimary')) $('jobPrimary').onclick=async ()=>{
  const st=$('jbStatus').value, next=JOB_NEXT[st]; if(!next) return;
  if(next==='done'){
    const hrs=curWorks.reduce((a,w)=>a+(+w.hours||0),0);
...
function jobHead(){
  ...
  $('jobHeadEcon').innerHTML=canWrite()
    ...
    : ('<div class="te-k">Записано</div>'
      +'<div class="te-v">'+hrs.toFixed(hrs%1?1:0)+' ч</div>'
      +'<div class="te-s">'+works+'</div>');
  // Дата словами рядом с системным полем: «09/11/2026» читается и как
  // 9 ноября, и как 11 сентября — формат берётся из браузера, не из страницы.
  if($('jbDateHint')) $('jbDateHint').textContent=$('jbDate').value?dateHuman($('jbDate').value):'';
  if($('jbDueHint')) $('jbDueHint').textContent=$('jbDue').value?dateHuman($('jbDue').value):'';
  if(document.querySelector('.view-job.job-task')) buildJobRead();
}
...
function renderJobWorks(){ const box=$('jbWorks'); box.innerHTML='';
  ...
  curWorks.forEach((w,i)=>{ const d=document.createElement('div'); d.className='eqitem';
    ...
    // Тариф, ручная цена и сумма строки — органы управления деньгами.
...
// было ни одной. Снимок «до» отвечает на «оно уже было сломано», «после» —
// на «вы не сделали», «дефект» — на «деталь была бракованной».
//
// Главное решение здесь не про интерфейс, а про размер: снимок с телефона
// весит 3–12 МБ, и в поле на слабой связи он просто не уходит. Ужимаем до
// 1600 px по длинной стороне и JPEG 0.8 — это ~250 КБ, разницы на экране
// не видно, а отправка перестаёт быть лотереей. Перекодирование заодно
...
async function shrinkImage(file){
  // createImageBitmap с imageOrientation учитывает поворот из EXIF —
  // без него снятое вертикально приезжает лёжа.
  let bmp;
  try{ bmp=await createImageBitmap(file,{imageOrientation:'from-image'}); }
  catch(e){ bmp=await createImageBitmap(file); }
  // Ограничиваем не только сторону, но и БАЙТЫ. Мелкая деталь — трещина,
  // шильдик — сжимается плохо, и снимок «по стороне 1600» иногда весит
  // за мегабайт. На слабой связи это снова лотерея, поэтому при перевесе
  // сначала опускаем качество, потом сторону.
  const steps=[[PH_MAX,PH_Q],[PH_MAX,0.6],[1200,0.6]];
  let blob=null;
  for(const [side,q] of steps){
    ...
    if(!blob) break;
    if(blob.size<=PH_TARGET) break;
  }
  if(bmp.close) bmp.close();
  if(!blob) throw new Error('не удалось пережать снимок');
  return blob;
}
...
async function renderJobPhotos(){
  ...
  let urls={};
  try{
    const {data}=await sb.storage.from('job-photos').createSignedUrls(jobPhotos.map(p=>p.path),3600);
...
async function addJobPhotos(files,kind){
  ...
  // По одному, а не пачкой: на слабой связи параллельные отправки мешают
  // друг другу и все разом отваливаются по таймауту.
  for(let i=0;i<files.length;i++){
    phBusy=i+1;
    phState(files.length>1?('Отправляю '+(i+1)+' из '+files.length+'…'):'Отправляю снимок…');
    try{
      const blob=await shrinkImage(files[i]);
      const path='jobs/'+jobEditId+'/'+Date.now()+'-'+Math.random().toString(36).slice(2,8)+'.jpg';
      const up=await sb.storage.from('job-photos').upload(path,blob,{contentType:'image/jpeg'});
      if(up.error) throw up.error;
      // Файл без строки — мусор, которого приложение не увидит: если
      // запись не прошла, убираем и файл.
      ...
      await loadJobPhotos(); phState('Не отправлено: '+(e.message||e),true);
      return;
    }
  }
  phBusy=0; phState('');
...
async function delJobPhoto(id){
  const p=jobPhotos.find(x=>x.id===id); if(!p) return;
  if(!await confirmDialog('Удалить снимок «'+(PH_LABEL[p.kind]||p.kind)+'»?',{danger:true,okText:'Удалить'})) return;
  try{
    const {error}=await sb.from('job_photos').delete().eq('id',id);
    if(error) throw error;
    try{ await sb.storage.from('job-photos').remove([p.path]); }catch(e){}
    await loadJobPhotos();
  }catch(e){ phState('Не удалось удалить: '+(e.message||e),true); }
}
(function(){
  const add=$('jbPhotoAdd'), inp=$('jbPhotoFile'); if(!add||!inp) return;
  let kind='defect';
  add.querySelectorAll('[data-ph]').forEach(b=>b.onclick=()=>{
    if(!jobEditId){ phState('Сначала создай заявку',true); return; }
    kind=b.dataset.ph; inp.value=''; inp.click(); });
...
// сворачивают, звонят, связь пропадает, и заполненная форма исчезает
// молча, без единого слова. Существующая заявка сохраняется сама, а
// кнопка остаётся только для создания новой — там сохранять ещё нечего.
//
// Состояние показываем словами рядом с шапкой: «сохраняю…», «сохранено»,
...
async function saveJobNow(){
  ...
  // переставляли бы строки местами. Пришедшее во время записи изменение
